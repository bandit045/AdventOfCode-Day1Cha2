public class Main {
    static String[] name = {
        ...
        static String[] preradjeniStringSaBroj = new String[name.length];
        public static void main(String[] args) {

            int[] nizZaSkladistenjeBrojaZaSabiranje = new int[preradjeniStringSaBroj.length];

            //Kopira se array iz name u preradjeniStringSaBroj array za rad
            for (int i = 0; i < preradjeniStringSaBroj.length; i++) {
                preradjeniStringSaBroj[i] = getClearStringFromFirstPhase(i, name);
            }

            //Vrsi se brisanje char-a kroz petlju koji su ostali nakon menjanja: "one" za 1
            for (int i = 0; i < preradjeniStringSaBroj.length; i++) {
                preradjeniStringSaBroj[i] = getOnlyIntInWayOfString(i, preradjeniStringSaBroj);
            }

            //Dodaje se jos po jedan broj kroz iteraciju i uklanja se visak u String array
            for (int i = 0; i < preradjeniStringSaBroj.length; i++) {
                preradjeniStringSaBroj[i] = ifThereIsOnlyOneIntCharMakeThemTwo(i,preradjeniStringSaBroj);
            }

            for (int i = 0; i < preradjeniStringSaBroj.length; i++) {
                nizZaSkladistenjeBrojaZaSabiranje[i] = Integer.parseInt(preradjeniStringSaBroj[i]);
            }

            //Prestavlja ispis array of stringa preradjenog
            for (int i = 0; i < nizZaSkladistenjeBrojaZaSabiranje.length; i++) {
                System.out.println("Pozicija " + (i+3) + " je: " + nizZaSkladistenjeBrojaZaSabiranje[i]);
            }

            //Sabiranje brojeva finale####################################################
            int sum = 0;
            for (int i = 0; i < nizZaSkladistenjeBrojaZaSabiranje.length; i++) {
                sum+= nizZaSkladistenjeBrojaZaSabiranje[i];
            }
            System.out.println("Krajnji rezultat je: "+ sum);
            System.out.println(nizZaSkladistenjeBrojaZaSabiranje.length);
    }

    //Menjaju se tekstualni brojevi za numericke
    public static String getClearStringFromFirstPhase(int positionOfElementInArray, String[] nameOfArray) {
        return nameOfArray[positionOfElementInArray].replaceAll("one", "1").replaceAll("two","2").replaceAll("three","3").replaceAll("four","4").replaceAll("five","5").replaceAll("six", "6").replaceAll("seven","7").replaceAll("eight","8").replaceAll("nine", "9");
    }

    //Brisu se slova i whitespace iz stringa i ostaju samo brojevi
    public static String getOnlyIntInWayOfString(int positionOfElementInArray, String[] nameOfArray) {
        String outsideVariableForLoop = "";
        for (char c : new char[]{'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z'}) {
            String temp = nameOfArray[positionOfElementInArray].replace(c,' ');
            nameOfArray[positionOfElementInArray] = temp;
            temp = nameOfArray[positionOfElementInArray].replaceAll("\\s","");
            outsideVariableForLoop = temp;
        }
        return outsideVariableForLoop;
    }

    //Dodaju se jos po jedan broj na string ako je jedan i uklanja se sve osim zadnjeg i prvog
    public static String ifThereIsOnlyOneIntCharMakeThemTwo(int positionOfElementInArray, String[] nameOfArray) {
        String toReturn = nameOfArray[positionOfElementInArray];
        if (toReturn.length() == 1) {
            toReturn = toReturn.repeat(2);
        } else {
            char firstChar = toReturn.charAt(0);
            char lastChar = toReturn.charAt((toReturn.length()) - 1);
            toReturn = String.valueOf(firstChar) + String.valueOf(lastChar);
        }
    return toReturn;
    }
}